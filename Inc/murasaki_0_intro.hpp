 ...
 * @details
 * The IO function is packaged by class types. For example, The murasaki::Uart class canreceive a UART handle.
 *
 * \code
 ...
 * \details
 * The Murasaki class library is buit on FreeRTOS's preemptive configuration.
 * As a result, Murasaki is automatically aware with preemptive multi-task.
 * That means, Murasaki's classes don't use polling to wait for any event. Thus, a task
 * can do some job while other tasks are waiting for the end of the IO operation.
 *
 ...
 * \details
 * The synchronous IO and the  preemptive multi-task provide easier programming.
 * On the other hand, there is a possibility that two different tasks access
 * one peripheral simultaneously.
 * This kind of access messes the peripheral's behavior.
 *
 * To prevent this condition, some peripheral wrapping class has exclusive access mechanism
 * by mutex.
 *
 * By this mechanism, if two tasks try to transmit though one peripheral, one task
 * is kept waiting until the other finished to transmit. This is called as blocking behavior.
 *
 ...
 * internally.
 *
 * If the assertion failed, a message will be output to the debug port with the informaiton of the
 * souce line number and file name.
 *
 * This assertion is widlely used inside Murasaki class library.
 ...
 *
 * First, Musaraki's modules enable only when the relevant peripheral is generated by CubeIDE.
 * This allows you set the CubeIDE to generate only the used peripheral's source code.
 * Such the setting makes total source code smaller.
 * All unused drivers are invisible.
 * For example, if you don't enable the I2C pins on CubeIDE, application programmer cannot see
 * the I2C class in the Murasaki library.
 *
 * The Second part of the configurable characteristics is Murasaki itself.
 * The programmer can customize the Murasaki for example, heap memory size.
 *
 */

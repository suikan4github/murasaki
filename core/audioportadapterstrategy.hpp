 ...
 *
 *  @brief Strategy of the Audio device adapter.
 */
 ...
 * @details
 * Template class of the audio device adapter.
 */
class AudioPortAdapterStrategy : public murasaki::PeripheralStrategy {
 public:
   ...
   */
  virtual void StartTransferTx(uint8_t *tx_buffer,
   ...
   * once after it starts.
   */
  virtual void StartTransferRx(uint8_t *rx_buffer,
                               unsigned int channel_len) = 0;
  ...
  /**
   * @brief Return the bit count to shift the 1.31 fixed point data to the DMA
   * required format.
   ...
   * right aligned in 32bit DMA data ( as integer ), some peripheral places the
   * data as left aligned in 32bit DMA data ( as fixed point ).
   *
   * This kind of the mismatch will be aligned by audio frame work. This member
   * function returns how many bits have to be shifted from left aligned data to
   * the DMA required format..
   *
   * If peripheral requires the left aligned format, this function should return
   * 0.
   */
  ...
  /**
   * @brief Return the bit count to shift the DMA format to the left
   * aligned 1.31 format. .
   * @return 0..32. The unit is [bit]
   * @details
   * This is needed because of the variation of the DMA format.
   ...
   *
   * This kind of the mismatch will be aligned by audio frame work. This member
   * function returns how many bits have to be shifted from the DMA format to
   * the left aligned data.
   *
   * If peripheral DMA uses the left aligned format, this function should return
   * 0.
   */
   ...
   * @details
   * If the DMA interrupt doesn't have the explicit phase information, need to
   * override to detect it inside this function.
   *
   * By default, this function assumes the DMA phase is given though the
   * interrupt handler. So, just pass the input parameter as return value.
   */
  virtual unsigned int DetectPhase(unsigned int phase) { return phase; }

  /**
   * @brief Handling error report of device.
   * @param ptr Pointer for generic use. Usually, points a struct of a device
   * control
   * @return true if ptr matches with device and handle the error. false if ptr
   * doesn't match
   * @details
   ...
   * @return true : half word swap required. false :harlf word swap is not
   * required.
   * @details
   * Display whether the half word (int16_t) swap is required or not.
   *
   * Certain architecture requires to swap the upper half word and lower half
   * word inside a word (int32_t ). In case this is required before copying to
   * TX DMA buffer or after copying from RX DMA buffer, return true. Otherwise,
   * return false.
   *
   * The return value doesn't affect to the endian inside half word.
   * This display will be ignored if the audio sample size is half word
   * (int16_t) or byte (int8_t).
   */
